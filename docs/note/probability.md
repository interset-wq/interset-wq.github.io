...
- 确定性现象 例如：每天早晨太阳从东方升起；水在标准大气压下加温到100oC沸腾
...
??? question "例题——样本空间"
    ...
    - $E_4$ ：任选一人，记录他的身高(m)和体重(kg)．$\varOmega = \{ (h,g) | 0 < h < 3, 0 < g < 400 \}$
...
- 交换律、结合律
- 分配律
    - $A \cup (B \cap C) = (A \cup B)(A \cup C)$
    - $A \cap (B \cup C) = AB \cup AC$
- 对偶律
    - $\overline{ A \cup B } = \overline{A} \cap \overline{B}$ (和的逆=逆的积)
    - $\overline{ A \cap B } = \overline{A} \cup \overline{B}$ (积的逆=逆的和) 
...
<!-- 例5 (会面问题) 两人相约7点到8点在某地会面，
先到者等候另一人20分钟后就可离去，试求这两人
能会面的概率．
解 以 x，y分别表示两人到达时刻 (7点设为零时刻)，
y
则会面的充要条件 为 x - y \leq 20，这是一几何概率问题， 60
可能的结果全体是边长为60的正方形里的点，能会面的
20
x
o
602 - 402 5 20 60
...
- **有限可加性** (可推广到可列可加性) 设 $A_1 ，A_2 ，...，A_n$ 是 $n$ 个两两互不相容的事件，即 $A_i A_j = \varnothing (i \not ={j} ),i,j = 1, 2, ..., n$，则有$P(A_1 \cup A_2 \cup ... \cup A_n) = P(A_1) + P(A_2) + ... + P(A_n)$
- **事件差** A，B 是两个事件，则 $P(A - B) = P(A) - P(AB)$
- **单调性** 若事件$A \supset B$ ，则 $P(A) \ge P(B)$
- **加法公式** 对任意两事件A，B，有 $P(A \cup B) = P(A) + P(B) - P(AB)$，该公式可推广到任意 n 个事件 $A_1 ，A_2 ，...，A_n$ 的情形．$P(A \cup B \cup C) = P(A) + P(B) + P(C) - P(AB) - P(BC) - P(AC) + P(ABC)$
- **互补性** $P(\bar{A}) = 1 - P(A)$
...
<!-- 例1 设10件产品中有3件次品，现无放回的抽取2件，求在第一次抽到
次品的条件下，第二次抽到次品的概率．
P(A A ) C2 C2 2
解法1 A ={第 i 次抽到次品}，则 P(A | A ) = 1 2 = 3 10 =
i 2 1
P(A ) 3 9
1 10
2
解法2 A ={第 i 次抽到次品}，则 P(A | A ) = 缩小样本空间
i 2 1
9
注 区分条件概率与两事件同时发生概率的不同．  -->

设 $A，B \subset \varOmega$，若 $P(B) > 0$，则$P(AB) = P(B) \cdot P(A | B)$．若 $P(A) > 0$，则 $P(AB) = P(A) \cdot P(B | A)$．
...
解 A = {第 i 次取到白球}，i = 1，2，则 A = {第1次取到黑球}，
i 1
P(A ) = P(A $\varOmega$)= P(A  (A \cup A )) = P(A A \cup A A )
2 2 2 1 1 1 2 1 2
...
例3 (摸奖问题) 盒子里有 n 个球，其中n -1个白球，1个黑球．n 个人
依次取一个球，不放回．问第 i 个人取到黑球的概率．
解 A = {第 i 个人摸到黑球}， i = 1，2，...，n
i
1
P(A ) =
1
n
n -1 1 1
P(A ) = P(A A ) = P(A )P(A | A ) = \cdot  =
2 1 2 1 2 1
n n -1 n
P(A ) = P(A A ... A A ) = P(A )P(A | A )P(A | A A )...P(A | A ... A )
n 1 2 n-1 n 1 2 1 3 1 2 n 1 n-1
n -1 n - 2 n - 3 1 1
= \cdot  \cdot  ... =
n n -1 n - 2 n - (n -1) n  -->
{==全概率公式==}
