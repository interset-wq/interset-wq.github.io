...
### 4.4  指数分布 $X \sim E(\lambda)$ 或 $X \sim Exp(\lambda)$
...
### 4.6  二维正态分布 $(X ,  Y ) \sim N(\mu _1 ,  \mu _2 ；\sigma _1 ^2,  \sigma _2 ^2；\rho)$
...
???+ example "独立同分布的中心极限定理"
    ...
    **解** 设$ X_i$ 表示第 i 袋食品的净重,  则 $X_1 ,  X_2 ,  ...,  X_100$ 独立同分布,  且 $E(X_i ) = 100,  D(X_i ) = 16$,  而一箱净重 $X = \sum _{i=1} ^100 X_i$ 
